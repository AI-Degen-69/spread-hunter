...
trial (U32) or volume trial (U36) is configured (env HUNTER_DEPTH_TRIAL_USD /
HUNTER_VOLUME_TRIAL_USD), the loop must pass the bar through so adopted
...
def test_rank_cmd_is_plain_when_no_trials_are_configured(monkeypatch):
    ...
    monkeypatch.delenv("HUNTER_DEPTH_TRIAL_USD", raising=False)
    monkeypatch.delenv("HUNTER_VOLUME_TRIAL_USD", raising=False)
    cmd = _rank_cmd(20)
...
def test_rank_cmd_appends_configured_trials(monkeypatch):
    monkeypatch.setenv("HUNTER_DEPTH_TRIAL_USD", "750")
