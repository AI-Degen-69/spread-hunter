...
**Verdict.** LIVE as a measurement. The unit is now dollars; whether the $120 budget is the right number is a separate question that dollar gate replay validation's replay answers.
...
**Question.** The pairs-only rule (pairs-only completion and exit rule) fixes edge per fill, but fill rate is the other binding constraint (28 fills/112h), and it is bounded by the ELIGIBLE universe: the live pipeline scores 205 markets and picks 2. The fleet needs more eligible markets to accumulate samples, but every gate loosening must be evidence-first. Which gate actually binds — and does the licensed depth trial, plus a volume trial, add eligible markets?
...
**Question.** Is the pairs-only rule (pairs-only completion and exit rule) positive-EV on the current sample per its pre-registered KPI, and do the KPI's two payoff constants hold on the rule's own recorded decisions?
