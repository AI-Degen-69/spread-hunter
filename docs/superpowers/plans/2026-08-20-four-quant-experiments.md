...
### Task 5: Phase 4 — cross-venue toxic-flow breaker
...
**Spec coverage:** Phases 1–4 map to Tasks 1–5; cross-cutting (state reader, write module, four research files, session numbering, no live orders) is in Global Constraints. Phase 4's `PARKED` research verdict (zero sub-second book gaps) is recorded; the phase may be re-opened if a fast series enters the universe — noted, not silently skipped.
